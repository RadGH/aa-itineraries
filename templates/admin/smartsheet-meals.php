...
$base_url = admin_url('admin.php?page=ah-smartsheet-meals');

// Visiting this URL loads data from smartsheet
$sync_url = add_query_arg(array('ah_sync_meals' => 1), $base_url);

// Get the last sync time
$sync_date = get_option( 'ah_meals_last_sync', false );

// Get sheet settings
$sheet_id = AH_Smartsheet_Sync_Meals()->get_sheet_id();
$column_ids = AH_Smartsheet_Sync_Meals()->get_column_ids();
$sheet_url = AH_Smartsheet_Sync_Meals()->get_smartsheet_permalink();
$columns = AH_Smartsheet_Sync_Meals()->columns;

// Get meals from a prior Smartsheet sync
$meal_list = AH_Smartsheet_Sync_Meals()->get_stored_meal_list();
?>

<div class="wrap">
	
	<h1>Smartsheet - Sync Meals</h1>
				...
				<div id="postbox-container-1" class="ah-postbox-sidebar postbox-container">
					...
					<div class="ah-postbox-sticky">
						...
						<div class="postbox">
							...
							<div class="inside">
								<p>Updates the meals list with current information from the meals master spreadsheet.</p>
				...
				<div id="postbox-container-2" class="ah-postbox-main postbox-container">
					...
					<div class="postbox">
						...
						<div class="inside">
							...
							<input type="hidden" name="ah-action" value="<?php echo wp_create_nonce('save-meal-data'); ?>">
							...
							<div class="ah-admin-field">
								...
								<div class="ah-field-list">
									<?php
									/* Each column name must be defined in sync-meals.php, $this->columns */
									foreach( $columns as $key => $title ) {
										$value = $column_ids[ $key ] ?? '';
										?>
										<label for="ah-meal-col-<?php echo $key; ?>"><?php echo $title; ?>:</label>
										<div class="ah-field">
											<input type="text" name="ah[column_ids][<?php echo $key; ?>]" id="ah-meal-col-<?php echo $key; ?>" value="<?php echo esc_attr($value); ?>">
										</div>
										<?php
									}
									?>
